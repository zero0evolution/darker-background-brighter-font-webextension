	...
	propertyInfo["background-image"] = {
		...
		transFunc:function(obj,getStyleObj,setStyleObj,propertyVal,property){
			...
			if(typeof(optionInfo.bgImgBrightness.value) === "number"){
				newPropertyVal = propertyVal.replace(
					/url\(([^\(^\)]+?)\)/img,function(matchStr,p1){
						changeFlag = true
						var replaceStr = bgImgUrlBrightnessTrans(
							matchStr,optionInfo.bgImgBrightness.value)
						return(replaceStr)
					}
				)
			}
			if(changeFlag){
				setStyleObj.setProperty(
					"filter",
					getStyleObj.getPropertyValue("filter")+" drop-shadow(0px 0px 1px white)"
				)
			}
...
var imgElemFunc = function(elem){
	...
	if(typeof(optionInfo.imgBrightness.value) !== "number"){return(null)}
...
// 背景圖片亮度變暗功能
var bgImgUrlBrightnessTrans = function(urlStr,brightness){
	
	var blackBgOpacity = 1-brightness
	var bgRgbaStr = "rgba(0, 0, 0, "+String(blackBgOpacity)+")"

	var newBgImgStr = (
		"linear-gradient("+bgRgbaStr+", "+bgRgbaStr+")"+
		", "+urlStr
	)
...
var imgElemBrightnessTrans = function(elem,brightness){
	...
	var elemFilterStr = elem.style.getPropertyValue("filter")
	...
	var newElemFilterStr = elemFilterStr.replace(
		/brightness\((.*?)\)/i,function(matchStr,elemBrightness){
			...
			// var newBrightnessStr = String(Number(elemBrightness)*brightness)
			
			styleChangeFlag = true
			// return("brightness("+newBrightnessStr+")")
	...
	newElemFilterStr+=" drop-shadow(white 0px 0px 1px)"
	elem.style.setProperty("filter",newElemFilterStr)
